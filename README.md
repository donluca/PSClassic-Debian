...
- **No bootscreen:** I've decided to use a "safe" kernel as default which has the framebuffer console disabled because it was causing kernel panics and bootloops. If you feel adventurous, you can flash the bootscreen.img kernel but know that this might lead to having kernel panics during the boot process and even a bootloop, forcing you to use fastboot to recover. To do this, delete debian.img in the kernel folder inside your USB drive and rename bootscreen.img to debian.img. Finally create a file or folder named BOOT at the root of your USB drive so that it will force the flashing of the new "bootscreen" kernel. Lastly, boot into Debian and reboot your PSC to stock firmware, let it boot completely to the main screen and connect your USB drive and let it do its job.

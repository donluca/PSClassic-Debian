...
- **Enable the boot screen:** If you feel adventurous, you can flash the bootscreen.img kernel but know that this might lead to having kernel panics during the boot process and even a bootloop, forcing you to use fastboot to recover. To do this, delete debian.img in the kernel folder inside your USB drive and rename bootscreen.img to debian.img. Finally create a file or folder named BOOT at the root of your USB drive so that it will force the flashing of the new "bootscreen" kernel. Lastly, boot into Debian and reboot your PSC to stock firmware, let it boot completely to the main screen and connect your USB drive and let it do its job. Now you'll be able to see the boot process happen on your monitor.
...
-  **You're stuck in a bootloop due to a kernel panic during booting (the power led never blinks green):** This is an unfortunate event that happens thanks to the trainwreck that is the included video driver which is uncapable of correctly communicating with your monitor. To avoid this from happening, boot the PSC without the monitor connected and connect it after the boot process has finished (you'll notice that the green power led will stop blinking when the boot process is complete) or hook your PSC up to a different monitor or with an HDMI-DVI adapter to the DVI port of your monitor. This should only happen if you're using the bootscreen.img kernel and the only workaround is to give up the framebuffer console, meaning you won't see the boot process happening on screen, but at least you won't have kernel panics anymore and you'll be able to boot into Debian.

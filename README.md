...
-  **You're stuck in a bootloop due to a kernel panic during booting:** This is an unfortunate event that happens thanks to the trainwreck that is the included video driver which is uncapable of correctly communicating with your monitor. The only solution is to either boot the PSC without the monitor connected and connect it after the boot process has finished (you'll notice that the green power led will stop blinking when the boot process is complete) or hook it up to a different monitor. The only workaround is to give up the framebuffer console, meaning you won't see the boot process happening on screen, but at least you won't have kernel panics anymore and you'll be able to boot into Debian. To do this, delete debian.img in the kernel folder inside your USB drive and rename safe.img to debian.img. Finally create a file or folder named BOOT at the root of your USB drive so that it will force the flashing of the new kernel. Finally, reboot your PSC to stock firmware, let it boot completely and connect your USB drive and let it do its job.
